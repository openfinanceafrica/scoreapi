import json
from constants import EXPECTED_PAYMENT_DAY_MAXIMUM, EXPECTED_PAYMENT_DAY_MINIMUM, EXPECTED_PAYMENT_AMOUNT_MINIMUM
...
def validateScoreInput(input: ScoreInput):
    ...
    try:
        ...
        if currentDate:
            parser.isoparse(inputObj['currentDate'])
    except Exception as e:
        return '"currentDate" is invalid. The format should be full-date as defined by "RFC 3339, section 5.6" (https://tools.ietf.org/html/rfc3339#section-5.6). For example: "2023-03-08T07:15:32.42"'

    try:
        if type(inputObj['expectedPaymentDay']) != int and (inputObj['expectedPaymentDay'] < EXPECTED_PAYMENT_DAY_MINIMUM or inputObj['expectedPaymentDay'] > EXPECTED_PAYMENT_DAY_MAXIMUM):
            raise
    except Exception as e:
        print(e)
    ...
    try:
        if (type(inputObj['expectedPaymentAmount']) != int and type(inputObj['expectedPaymentAmount']) != float) or inputObj['expectedPaymentAmount'] < EXPECTED_PAYMENT_AMOUNT_MINIMUM:
            raise
    except Exception as e:
        print(e)
